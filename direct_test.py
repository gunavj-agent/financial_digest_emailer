#!/usr/bin/env python3
"""
Direct test script for the Financial Digest Emailer workflow.
This script explicitly sets environment variables to ensure they're properly loaded.
"""
...
logger.info(f"SMTP_SERVER: {os.environ.get('SMTP_SERVER')}")
logger.info(f"SMTP_USERNAME: {os.environ.get('SMTP_USERNAME')}")
logger.info(f"EMAIL_FROM: {os.environ.get('EMAIL_FROM')}")
...
async def test_workflow():
    ...
    try:
        ...
        email_data = EmailData.model_validate(email_data_dict)  # Using model_validate instead of parse_obj
